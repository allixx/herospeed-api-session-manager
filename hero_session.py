...
def session_login(host, port, credentials, timeout=TIMEOUT):
    ...
    with request.urlopen(req, timeout=timeout) as response:
        if response.status == 200 and response.reason == "OK":
            ...
            raise HTTPError(
                f"Invalid login-capabilities response {response.status} {response.reason}"
            )
    ...
    form_data = json.dumps(data).encode()
    req = request.Request(
        url=f"{host}:{port}/api/session/login", data=form_data, headers=HEADERS
    )
    ...
    with request.urlopen(req, timeout=timeout) as response:
        ...
        if response.status == 200 and response.reason == "OK":
            ...
            raise HTTPError(
                f"Invalid login response {response.status} {response.reason}"
            )
...
def session_logout(host, port, session_id, timeout=TIMEOUT):
    ...
    form_data = json.dumps(data).encode()
    req = request.Request(
        url=f"{host}:{port}/api/session/logout", data=form_data, headers=headers
    )
    ...
    with request.urlopen(req, timeout=timeout) as response:
        if response.status == 200 and response.reason == "OK":
            ...
            raise HTTPError(
                f"Invalid logout response {response.status} {response.reason}"
            )
